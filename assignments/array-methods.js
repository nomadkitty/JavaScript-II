...
let allCaps = [];
allCaps = runners.map(function(arrItem){
...
let largeShirts = [];
largeShirts = runners.filter(function(arrItem){
    return arrItem.shirt_size === "L";
})
...
ticketPriceTotal = runners.reduce(function(total, CurrentItem){
    return total += CurrentItem.donation;
}, 0)
...
// Problem 1: sort the first name alphabetically 
let newFirstName = runners.map((arrItem) => arrItem.first_name)
console.log(newFirstName.sort());
// Problem 2
